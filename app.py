...
# Initialize session states (Single, clean key for two-way binding)
...
# Safe Callback helper function to update input cleanly
def load_into_analyzer(pwd):
...
with col_analyser:
    ...
    # Password Input - bound purely to key="password_input"
...
with col_generator:
    ...
    if st.session_state.generated_password:
        ...
        # Safe Callback button to test in analyser
        st.button("⚡ Test in Analyser", on_click=load_into_analyzer, args=(st.session_state.generated_password,))
    ...
    with hist_col1:
        ...
        if st.session_state.analysis_history:
            for i, entry in enumerate(st.session_state.analysis_history):
                ...
                with col_h2:
                    st.button("Load 🔍", key=f"load_ana_{i}", on_click=load_into_analyzer, args=(entry["password"],))
        else:
    ...
    with hist_col2:
        ...
        if st.session_state.generator_history:
            for i, pwd in enumerate(st.session_state.generator_history):
                ...
                with col_h2:
                    st.button("Load 🔍", key=f"load_gen_{i}", on_click=load_into_analyzer, args=(pwd,))
        else:

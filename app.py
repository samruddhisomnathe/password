...
# Custom CSS Injection for Premium Glassmorphic Design
st.markdown("""
<style>
...
/* Reset nested columns so they don't look nested */
...
/* Code block custom presentation */
...
def get_checklist_html(password):
    ...
    for label, passed in checks:
        ...
        <div class="check-item">
            <span class="check-icon {icon_class}">{icon_symbol}</span>
...
def get_strength_meter_html(score, password):
    ...
    <div class="meter-container">
        <div class="meter-label">
            <span>Strength: <span style="color: {text_color}; font-weight: 800;">{strength_label}</span></span>
            <span>{width}</span>
        </div>
        <div class="meter-bg">
            <div class="meter-fill" style="width: {width}; background: {gradient};"></div>
        </div>
    </div>
    """
...
def get_crack_times_html(crack_times):
    ...
    for title, time_val, description in scenarios:
        ...
        <div class="crack-card">
            <div class="crack-title">{title}</div>
            <div class="crack-time" style="color: {time_color};">{time_val}</div>
            <div style="font-size: 0.72rem; color: #64748b; margin-top: 4px;">{description}</div>
        </div>
        """
    html += '</div>'
    return html
...
def get_feedback_html(feedback):
    ...
    if suggestions:
        ...
        <div class="suggestion-box">
            ...
            <ul style="margin: 0; padding-left: 15px; color: #cbd5e1; font-size: 0.9rem;">
                {suggestions_list}
            </ul>
        </div>
        """
...
with col_generator:
    ...
    if gen_mode == "Random Characters":
        ...
        if st.button("Generate Passphrase"):
            st.session_state.generated_password = generate_passphrase(
    ...
    if st.session_state.generated_password:
        ...
        st.markdown('<p style="font-size:0.8rem; color:#64748b; margin-top:-10px; margin-bottom:15px;">💡 Click the copy icon in the top-right corner of the box to copy.</p>', unsafe_allow_html=True)
